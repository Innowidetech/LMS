...
export const updateUserInfo = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { name, email } = req.body as IUdateUserInfo;
      const userId = req.user?._id;
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!oldPassword || !newPassword) {
        return next(new ErrorHandler("Enter oldPassword and newPasswords", 400));
      }
...
export const getAllUsers = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const users = await getAllUsersService(); // Assuming this fetches all users
      res.status(200).json({
        success: true,
