...
export const getCourseByUser = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!courseExists) {
        return next(
          new ErrorHandler("You are not eligible to access this courses ", 404)
        );
      }
...
export const addQuestion = CatchAsyncError(async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (course.createdBy) {
      const createdByUser = course.createdBy as IUser;
  ...
  const html = await ejs.renderFile(path.join(__dirname, "../mails/add-question.ejs"), data);
...
export const addReview = CatchAsyncError(async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!courseExists) {
      return next(new ErrorHandler("You are not eligible to accesss this course", 404))
    }
